...
export default function DossierModal({ isOpen, onClose, course }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-6 bg-slate-950/90 backdrop-blur-md animate-in fade-in duration-300">
      <div className="bg-slate-900 w-full max-w-5xl max-h-[95vh] rounded-[2rem] md:rounded-[3rem] border border-white/10 overflow-hidden shadow-2xl flex flex-col">
        
        {/* Close Button */}
        <button 
          onClick={onClose} 
          className="absolute top-6 right-6 z-10 p-3 bg-black/50 hover:bg-white/10 rounded-full transition-colors"
        >
        ...
        {step === 1 && (
          <div className="flex flex-col lg:flex-row h-full overflow-y-auto lg:overflow-hidden">
            {/* LEFT: Visual Impact */}
            <div className="lg:w-1/2 h-64 lg:h-auto bg-slate-800 flex-shrink-0 relative">
              {flyerImage ? (
                <img 
                  src={flyerImage} 
                  alt={course.title} 
                ...
                <div className="w-full h-full flex flex-col items-center justify-center text-slate-600">
                  {isService ? <RocketLaunchIcon className="w-20 h-20 opacity-30" /> : <CubeIcon className="w-20 h-20 opacity-30" />}
                  <span className="text-[10px] font-black uppercase tracking-widest mt-4">No Visual Uplink</span>
                </div>
              )}
              {/* Type Badge */}
              <div className="absolute top-4 left-4 px-4 py-2 bg-blue-600/90 backdrop-blur-md text-[10px] font-black text-white uppercase tracking-widest rounded-full flex items-center gap-2 shadow-lg">
                {isService ? <RocketLaunchIcon className="w-4 h-4" /> : <CubeIcon className="w-4 h-4" />}
            ...
            <div className="lg:w-1/2 p-8 md:p-10 flex flex-col overflow-y-auto">
              ...
              <div className="mb-8">
                ...
                <h2 className="text-3xl md:text-4xl font-black text-white uppercase tracking-tighter italic leading-tight mt-2">
                  {course.title}
                </h2>
              </div>

              {/* Seller Info */}
              <Link to={`/profile/${course.seller?._id}`} onClick={onClose} className="flex items-center gap-3 mb-8 group">
                ...
                {isService ? (
                  ...
                  <div className="space-y-4">
                    <div className="flex flex-wrap gap-3 mb-4">
                      <div className="px-3 py-2 bg-slate-800 rounded-xl border border-white/5 flex items-center gap-2">
                        <BoltIcon className="w-4 h-4 text-blue-500" />
                        <span className="text-[10px] font-black text-white uppercase tracking-widest">{course.skuDetails?.skillLevel || 'Intermediate'}</span>
                      </div>
                      <div className="px-3 py-2 bg-slate-800 rounded-xl border border-white/5 flex items-center gap-2">
                        <ClockIcon className="w-4 h-4 text-blue-500" />
                        <span className="text-[10px] font-black text-white uppercase tracking-widest">{course.skuDetails?.duration || 'Flexible'}</span>
                      </div>
                    </div>
                    ...
                    {course.skuDetails?.roadmap?.length > 0 && (
                      <div className="space-y-3">
                        ...
                        {course.skuDetails.roadmap.map((step, i) => (
                          <div key={i} className="flex items-start gap-3">
                            <div className="w-6 h-6 rounded-lg bg-blue-600 flex items-center justify-center text-[10px] font-black text-white shrink-0">
                              {i + 1}
                            </div>
                            <span className="text-sm text-white font-medium italic">{step}</span>
                          </div>
                        ))}
                      </div>
                    )}
                  ...
                  <div className="space-y-4">
                    {course.skuDetails?.stockCount > 0 && (
                      <div className="px-4 py-3 bg-green-500/10 rounded-xl border border-green-500/20 flex items-center gap-2">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {course.skuDetails.specifications.map((spec, i) => (
                            <div key={i} className="p-3 bg-slate-800 rounded-xl border border-white/5">
                              <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest block mb-1">{spec.key}</span>
              ...
              <div className="p-4 bg-blue-500/5 rounded-xl border-l-4 border-blue-600 mb-8">
                ...
                <Link to="/communities" onClick={onClose} className="text-[10px] font-black text-blue-500 uppercase tracking-widest mt-2 inline-block hover:underline">
                  Enter the Pulse →
                </Link>
              </div>
              ...
              <div className="mt-auto pt-6 border-t border-white/5">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <span className="text-3xl font-black text-blue-500 tracking-tighter italic">
                ...
                {/* Purpose Selector */}
                <div className="mb-6">
                  <label className="block text-[9px] font-black text-slate-500 uppercase tracking-widest mb-3">Protocol Purpose</label>
                  <div className="grid grid-cols-2 gap-2">
                    {['Inquiry', 'Purchase', 'Support', 'Custom'].map(p => (
                      <button 
                        key={p}
                        onClick={() => setPurpose(p)}
                        className={`px-4 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border ${purpose === p ? 'bg-blue-600 border-blue-600 text-white shadow-lg' : 'bg-slate-800 border-white/5 text-slate-400 hover:text-white'}`}
                      >
                        {p}
                      </button>
                    ))}
                ...
                <button 
                  onClick={handleSync}
                  className="w-full py-5 bg-white text-slate-900 rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] hover:bg-blue-500 hover:text-white transition-all shadow-xl active:scale-95 flex items-center justify-center gap-3"
                >
